...
extern "C" {
 ...
 EXPORT_API void SetOnConferenceStatusUpdatedHandler(on_conference_status_updated::type handler) {
  ...
  EXPORT_API int Create(dolbyio::comms::native::conference_options* opts, dolbyio::comms::native::conference_info* conference) {
    return call { [&]() {
      ...
      no_alloc_to_c(conference, result);
    }}.result();
  }

  EXPORT_API int Join(dolbyio::comms::native::conference_info* ifs, dolbyio::comms::native::join_options* opts, dolbyio::comms::native::conference_info* res) {
    return call { [&]() {
      auto options = to_cpp<dolbyio::comms::services::conference::join_options>(opts);
      auto conference = to_cpp<dolbyio::comms::conference_info>(ifs);
      auto result = wait(sdk->conference().join(conference, options));
      no_alloc_to_c(res, result);
    }}.result();
  }

  EXPORT_API int Demo(bool spatial_audio, dolbyio::comms::native::conference_info* conference) {
    return call { [&]() {
  ...
  EXPORT_API int Listen(dolbyio::comms::native::conference_info* ifs, dolbyio::comms::native::listen_options* opts, dolbyio::comms::native::conference_info* res) {
    return call { [&]() {
      auto options = to_cpp<dolbyio::comms::services::conference::listen_options>(opts);
      auto conference = to_cpp<dolbyio::comms::conference_info>(ifs);
      auto result = wait(sdk->conference().listen(conference, options));
      no_alloc_to_c(res, result);
    }}.result();
  }

  EXPORT_API int GetCurrentConference(dolbyio::comms::native::conference_info* res) {
    return call { [&]() {
      auto conference = wait(sdk->conference().get_current_conference());
      no_alloc_to_c(res, conference);
    }}.result();
  }
